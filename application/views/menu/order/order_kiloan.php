...
<section class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1 class="m-0 text-dark">Menu Pesanan Kiloan</h1>
      </div>
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item"><a href="#">Home</a></li>
          <li class="breadcrumb-item active">Pesanan kiloan</li>
        </ol>
      </div>
    </div>
  </div>
      ...
      <table id="example1" class="table table-bordered table-striped table-responsive-md">
        ...
        <tbody>
          <?php foreach ($order as $data) { ?>
            <tr>
              <td>
                <a href="" data-toggle="modal" id="tgl" data-target="#modal-detail<?= $data->id_order_kiloan ?>" data-tooltip="tooltip" title="Tekan untuk melihat detail">
                  <?= $data->kode_order_kiloan ?>
                </a>
              </td>
              <?php if ($data->id_member == null) { ?>
                <td><?= $data->nama_pelanggan_kiloan ?></td>
              <?php } else { ?>
              ...
              <td><?= $data->status_kiloan ?></td>
              <td>
                <a href="#modal-edit<?= $data->id_order_kiloan ?>" class="btn btn-warning btn-xs float-left m-1" data-toggle="modal" data-tooltip="tooltip" title="Ubah Data"> <i class="fa fa-edit"></i> </a>
                <?php if ($this->fungsi->user_login()->id_level == 1) { ?>
                  <a href="<?= site_url('order/order_kiloan/del/') . $data->id_order_kiloan ?>" class="btn btn-danger btn-xs float-left tombol-hapus m-1" data-tooltip="tooltip" title="Hapus Data"><i class="fa fa-trash"></i> </a>
                <?php } ?>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="<?= site_url('order/order_kiloan/add') ?>" method="post" role="form">
          <!-- FORM BODY -->
          <!-- nama member -->
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" name="nama" placeholder="Customer name">
          </div>
          <!-- product -->
          <div class="form-group">
            <label>Jasa*</label>
            <select class="select2" name="idproduct" id="product" style="width: 100%;" required <?= set_value('product') ?>>
              ...
              <?php foreach ($product as $data) { ?>
                <option value="<?= $data->id_product_kiloan . '/' . $data->harga_product_kiloan . '/' . $data->durasi_kiloan ?>" <?= $data->status_kiloan == 2 ? 'disabled' : '' ?>><?= strtoupper($data->nama_product_kiloan) . ' - ' . number_format($data->harga_product_kiloan) ?></option>
              <?php } ?>
            </select>
          </div>
          <!-- parfume -->
          ...
          <!-- berat cucian -->
          <div class="form-group">
            <label> Berat* </label> <small class="float-right">/Kg</small>
            <input type="number" min="0" name="berat" id="berat" class="form-control" placeholder="weight" required <?= set_value('berat') ?>>
          </div>
          <!-- Total harga -->
          <div class="form-group">
            ...
            <div class="input-group">
              ...
              <input type="number" name="total" id="total" class="form-control" placeholder="Total Price" readonly <?= set_value('total') ?>>
            </div>
          </div>
          <!-- periode -->
          <div class="form-group">
            ...
            <div class="col-md-6">
              <div class="form-group">
                ...
                <div class="input-group">
                  ...
                  <input type="text" name="pembayaran" id="pembayaran" class="form-control" placeholder="Payment" id="pembayaran" <?= set_value('pembayaran') ?> required>
                </div>
              </div>
            </div>
            <!-- kembalian -->
            <div class="col-md-6">
              <div class="form-group">
                ...
                <div class="input-group">
                  ...
                  <input type="text" name="kembalian" id="kembalian" class="form-control" placeholder="Change" readonly <?= set_value('kembalian') ?>>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="modal-footer justify-content-between">
            ...
            <button type="submit" class="btn btn-primary" name="add">Save new</button>
          </div>
        </form>
      </div>
    </div>
...
<!-- MODAL EDIT -->
<?php foreach ($order as $data) {
  $pelanggan = $data->nama_pelanggan_kiloan;
  $berat = $data->berat_kiloan;
  $total = $data->total_harga_kiloan;
  $note = $data->keterangan_kiloan;
  $enddate = $data->tanggal_keluar_kiloan;
  $product = $data->nama_product_kiloan;
  $parfume = $data->varian_inventory;
  $promo = $data->nama_promo;
  $karyawan = $data->nama_karyawan;
  // $member = $data->id_member;
?>
  <div class="modal fade" id="modal-edit<?= $data->id_order_kiloan ?>">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form action="<?= site_url('order/order_kiloan/edit/' . $data->id_order_kiloan) ?>" method="post" role="form">
            <!-- FORM BODY -->
            <input type="hidden" name="idorder_k" value="<?= $data->id_order_kiloan ?>">
            <!-- code -->
            <div class="form_group">
              <label> Kode </label>
              <input type="hidden" name="idorder" value="<?= $data->id_order_kiloan ?>">
              <input type="text" class="form-control" name="code" value="<?= $data->kode_order_kiloan ?>" readonly>
            </div>
            <br>
            <!-- nama member -->
            ...
            <div class="form-group">
              <label> Berat </label> <small class="float-right">/Kg</small>
              <input type="number" min="0" name="berat" value="<?= $berat ?>" class="form-control" readonly>
            </div>
            <!-- Total harga -->
            <div class="form-group">
              <label> Total Harga </label> <small class="float-right">Rp.</small>
            ...
            <div class="modal-footer justify-content-between">
              <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
              <button type="submit" class="btn btn-primary" name="edit">Save new</button>
            </div>
          </form>
        </div>
      </div>
        ...
        <div class="modal-body">
          ...
          <div class="row invoice-info">
            <div class="col-sm-6 invoice-col">
              ...
              <?php } else { ?>
                <address>
                  <strong><?= ucwords($data->nama_pelanggan_kiloan) ?></strong>
                </address>
              <?php } ?>
            </div>
            <!-- /.col -->
